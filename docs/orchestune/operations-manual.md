...
- **トリガー**: `schedule`（毎時17分、`--apply`込みで実dispatch）+ `workflow_dispatch`
  （`apply`入力、既定`true`＝実dispatch。dry-run確認時は`apply: false`を明示指定する）
...
- **動作確認したいだけの場合は、まず`workflow_dispatch`で`apply: false`を明示指定してdry-run実行し、
  `selected`/`quota_slots_available`/`lock_changes`等のJSON出力を確認する。**
  （`apply`入力の既定は`true`のため、確認せずに手動実行すると実際にdispatchされる点に注意）
