...
### 追記（2026-07-07）: #217でも同種の事象が再発し、コード側を修正（#236 / PR #237）

パイロット運用中、`review-history-backend-service`（#217）も#220と全く同じ経緯（dispatcher
管理外での例外的な手動dispatch）を辿っていたことが判明した。人間が成果物のPR（#231）を
レビュー・マージした際、PR本文の`Closes #217`によりGitHubが自動的に#217をCloseしたところ、
上記と同じ理由で後続タスク（#218, `depends_on: review-history-backend-service`）が
`status:blocked`のまま昇格しなくなった。

「運用ルールで回避する」方針は1週間経たずに2回目の実発生を迎えた。Issue完了時にCloseするのは
GitHub上ごく普通の運用であり、都度気をつけ続けるのは非現実的と判断し、方針を転換して
コード側を修正した（`github.list_issues_by_label`に`state`引数を追加し、`status:done`の
検索のみ`state="all"`でclosedなIssueも含めるようにした）。`docs/orchestune/operations-manual.md`
の運用ルールもあわせて更新済み。

**教訓の更新**: 「根本原因が運用ミスにある場合はまず運用ルールで回避する」という初動判断は
妥当だったが、その運用ミスが「通常のGitHub運用（Issueを完了時にCloseする）そのもの」と
衝突する場合は、運用ルールでの回避は長続きしない。一度で判断せず、同種の事象が再発した
時点でコード修正に切り替える、という今回の判断プロセス自体は今後も踏襲してよいと思われる。
